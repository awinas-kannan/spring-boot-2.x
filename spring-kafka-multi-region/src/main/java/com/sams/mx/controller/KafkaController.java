package com.sams.mx.controller;
...
import com.sams.mx.service.KafkaProducerService;
...
//		uuid.addAll(Arrays.asList("1a38c1f2-a784-469a-bcf0-f49d5313f032"));
	...
	public String stringProducer(@PathVariable("Message") String message) {
		for (int i = 0; i < 50; i++) {
			kafkaProducerService.sendMessage(message, message + i);
		}
	...
	public String stringProducerTest(@PathVariable("Message") String message) {
		for (String uuid1 : uuid) {
			kafkaProducerService.sendMessage(uuid1, message + "  ---  " + uuid1);
		}
