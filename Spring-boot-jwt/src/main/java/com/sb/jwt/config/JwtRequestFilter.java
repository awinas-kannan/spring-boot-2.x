...
public class JwtRequestFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		final String requestTokenHeader = request.getHeader(Constants.AUTH_HEADER);
		String username = null;
		String jwtToken = null;
		// Remove Bearer word and get only the Token
		if (Optional.ofNullable(requestTokenHeader).isPresent()
				&& requestTokenHeader.startsWith(Constants.TOKEN_PREFIX)) {
		...
		if (Optional.ofNullable(username).isPresent()
			...
			log.info("Valid Token {} **************",jwtUtil.validateToken(jwtToken, userDetails));
